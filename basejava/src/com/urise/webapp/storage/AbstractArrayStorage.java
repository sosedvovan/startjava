...
//здесь родитель параметризуется <Integer>-ом, тк SearchKey здесь это Integer index
    ////SearchKey в хранилищах - (обычном и сортированном массиве ) бедет Integer index - индекс обычного массива
public abstract class AbstractArrayStorage /*implements Storage*/extends AbstractStorage<Integer> {
    ...
    //------------------------------------------------------------------------------------------------------------------
    // Метод update(),который в родителе производит проверки  а  этот doUpdate() его дошаблонит:
  ...
  //----------------------------------------------------------------------------------------
  //реализуем здесь абстрактный метод из class AbstractStorage<SK> (но и пробросить его можно тк этот класс абстакт)
    @Override
