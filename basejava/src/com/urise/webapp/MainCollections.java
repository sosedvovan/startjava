      ...
      /**
        ...
        */
        ...
        for (String uuid : map.keySet()){//keySet()-взять ключ
            System.out.println(map.get(uuid));//get(uuid)-взять uuid из объекта RESUME_? кот. соотв. ключу
        }
        ...
        //долее пример вложенного статического класса:
        //выделили правую часть присвоения и Alt+Ctrl+V это экстракт вариэйбл - увидели тип RESUME_?
 ...
 * когда мы туда что-то кладем-put()- то сначала находим номер карзинки
 ...
 * в TreeSet.
 *
 * Оптимально, когда в каждой корзине лежит по одной паре(хорошая балансировка-О от 1). Есть LoadFactor = 0,75 -
 * те когда HashMap заполнен больше, чем на 75%, происходит двукратное увеличение его размера
 * для уменьшения колличества коллизий(О от n)(О от ln по основанию n - в 8-ой Java).
 */
 ...
 * --> крассивее код будет (инкапсулируем какие-то методы и данные связанные по смыслу, которые больше
