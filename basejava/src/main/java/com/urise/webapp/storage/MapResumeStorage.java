...
//здесь идея в том чтобы ключ(uuid) брать из объекта Resume r с помощью геттера: r.getUuid(),
//те в getSearchKey() подаем String uuid а возвращаем Resume r из которого потом берем uuid обратно с помощью геттера: r.getUuid()
